...
La documentation est donc **générée de manière assistée par IA** à partir des fichiers textuels présents dans le dossier `/docs`.
...
- [2) Navigation & ergonomie générale](#2-navigation--ergonomie-générale)
- [3) Référentiels & constantes (projet)](#3-référentiels--constantes-projet)
- [4) Écrans “Clients”](#4-écrans-clients)
  - [4.1 Liste / Table clients — FI_Volet_Table_Client](#41-liste--table-clients--fi_volet_table_client)
  - [4.2 Fiche client — FI_Volet_Fiche_Saisie_Client](#42-fiche-client--fi_volet_fiche_saisie_client)
- [5) Écrans “Produits”](#5-écrans-produits)
  - [5.1 Liste / Table produits — FI_Volet_Table_Produit](#51-liste--table-produits--fi_volet_table_produit)
  - [5.2 Fiche produit — FI_Volet_Fiche_Saisie_Produit](#52-fiche-produit--fi_volet_fiche_saisie_produit)
- [6) Flux “Commande” → “Facture”](#6-flux-commande--facture)
  - [6.1 Fiche commande — FI_Volet_Fiche_Saisie_Commande](#61-fiche-commande--fi_volet_fiche_saisie_commande)
  - [6.2 Fiche facture — FI_Volet_Fiche_Saisie_Facture](#62-fiche-facture--fi_volet_fiche_saisie_facture)
- [7) États / impressions](#7-états--impressions)
  - [7.1 État clients — ETAT_Clients](#71-état-clients--etat_clients)
  - [7.2 État facture — ETAT_Facture](#72-état-facture--etat_facture)
- [8) Parcours utilisateur (synthèse)](#8-parcours-utilisateur-synthèse)
- [9) Points d’attention (qualité fonctionnelle)](#9-points-dattention-qualité-fonctionnelle)
...
1. Ouvrir `FI_Volet_Table_Client`
2. Rechercher un client (filtres) ou afficher tout
3. Double-clic / action → ouvre `FI_Volet_Fiche_Saisie_Client` dans un onglet
4. Modifier infos + adresse (validation email, affichage carte)
5. Sauver
6. Depuis la fiche client, consulter commandes/factures et ouvrir le détail

### Parcours “Commande”
1. Depuis un client, ouvrir une commande (`FI_Volet_Fiche_Saisie_Commande`)
2. Ajouter lignes (référence/quantité, saisie assistée)
3. Vérification références
4. Calcul automatique des totaux
5. Sauver
6. Générer facture (bouton/traitement) → création facture liée

### Parcours “Facture”
1. Ouvrir `FI_Volet_Fiche_Saisie_Facture`
2. Visualiser client, entête facture, lignes issues de la commande
3. Imprimer via `ETAT_Facture`
